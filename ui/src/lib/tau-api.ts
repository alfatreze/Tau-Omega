...
import type { Comparison, Core, DuplicateGroup, LibrarySummary, MediaScan, Plan, Setting } from './types';
...
export const executeSync = (sources: string[], destination: string, confirmation: string, manifestPath: string, embedCovers: boolean) => invoke<{ copied: number; unchanged: number; index_path: string; warnings: string[] }>('execute_sync', { sources, destination, confirmation, manifestPath, embedCovers });
