...
export const executeSync = (sources: string[], destination: string, confirmation: string, manifestPath: string, embedCovers: boolean, jobId: string) => invoke<{ copied: number; unchanged: number; index_path: string; warnings: Warning[] }>('execute_sync', { sources, destination, confirmation, manifestPath, embedCovers, jobId });
...
export const executeCoreCopy = (source: string, destination: string, confirmation: string, manifestPath: string, jobId: string) => invoke<{ copied: number; unchanged: number; index_path: string }>('execute_core_copy', { source, destination, confirmation, manifestPath, jobId });
export const executeCoreMove = (source: string, destination: string, confirmation: string, backupPath: string, manifestPath: string, jobId: string) => invoke<{ copied: number; unchanged: number; index_path: string }>('execute_core_move', { source, destination, confirmation, deleteConfirmation: confirmation, backupPath, manifestPath, jobId });
