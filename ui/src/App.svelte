<script lang="ts">
  ...
  import { compareMedia, errorMessage, executeCoreCopy, executeCoreMove, executeSync, exportPlaylist, findDuplicates, inspectCard, planCoreCopy, planSync, readJournal, readPersistedSettings, scanMedia, summarizeLibrary } from './lib/tau-api';
  ...
  async function scanPlaylists() { try { playlistResult = await scanMedia(playlistPath); playlistNotice = `${playlistResult.playlists.length} playlists found. Nothing was changed.`; } catch (error) { playlistResult = null; playlistNotice = `Could not scan this folder: ${errorMessage(error)}`; } }
  ...
  async function inspectLibrary() { try { librarySummary = await summarizeLibrary(libraryPath); libraryNotice = 'Library inspected. Nothing was changed.'; } catch (error) { librarySummary = null; libraryNotice = `Could not inspect this folder: ${errorMessage(error)}`; } }
