<script lang="ts">
  ...
  import { compareMedia, executeCoreCopy, executeCoreMove, executeSync, exportPlaylist, findDuplicates, inspectCard, planCoreCopy, planSync, readJournal, readPersistedSettings, scanMedia, summarizeLibrary } from './lib/tau-api';
  ...
  async function loadSettings() { try { settings = await readPersistedSettings(settingsPath); settingsNotice = `${settings.length} persisted values loaded. Nothing was changed.`; } catch (error) { settings = []; settingsNotice = `Could not read settings: ${String(error)}`; } }
  async function scanPlaylists() { try { playlistResult = await scanMedia(playlistPath); playlistNotice = `${playlistResult.playlists.length} playlists found. Nothing was changed.`; } catch (error) { playlistResult = null; playlistNotice = `Could not scan this folder: ${String(error)}`; } }
  async function scanDuplicates() { problemsLoading = true; try { duplicateGroups = await findDuplicates(problemsPath); problemsNotice = `${duplicateGroups.length} duplicate groups found. Nothing was changed.`; } catch (error) { duplicateGroups = null; problemsNotice = `Could not scan this folder: ${String(error)}`; } finally { problemsLoading = false; } }
  async function inspectLibrary() { try { librarySummary = await summarizeLibrary(libraryPath); libraryNotice = 'Library inspected. Nothing was changed.'; } catch (error) { librarySummary = null; libraryNotice = `Could not inspect this folder: ${String(error)}`; } }
  async function loadJournal() { try { const journal = await readJournal(journalPath) as { state?: string; plan?: { files?: number }; result?: { copied?: number } }; jobs = [{ kind: 'Loaded journal', status: journal.state === 'completed' ? 'Completed' : journal.state ?? 'Unknown', detail: `${journal.result?.copied ?? 0} copied · ${journal.plan?.files ?? 0} planned` }, ...jobs]; journalNotice = 'Journal loaded. Nothing was changed.'; } catch (error) { journalNotice = `Could not load journal: ${String(error)}`; } }
  async function exportSelectedPlaylist() { try { await exportPlaylist(playlistPath, selectedPlaylist, playlistOutput); playlistNotice = `Exported ${selectedPlaylist}. Nothing in the source library was changed.`; } catch (error) { playlistNotice = `Could not export playlist: ${String(error)}`; } }
  ...
  async function openFolder() { if (!path.trim()) { notice = 'Enter a staging-card folder path first.'; return; } try { cores = await inspectCard(path); notice = `${cores.length} core${cores.length === 1 ? '' : 's'} found. This is a read-only inspection.`; } catch (error) { notice = `Could not inspect this folder: ${String(error)}`; cores = []; } }
  async function makePlan() { plan = null; syncNotice = ''; try { plan = await planSync(sources.split('\n'), destination, embedCovers); syncNotice = `Plan ${plan.id} is ready for review. Nothing has been written.`; } catch (error) { syncNotice = `Could not make a plan: ${String(error)}`; } }
  async function runSync() { if (!plan) return; try { const result = await executeSync(sources.split('\n'), destination, plan.id, manifestPath, embedCovers); syncNotice = `Verified: ${result.copied} copied, ${result.unchanged} unchanged. Index: ${result.index_path}`; jobs = [{ kind: 'Library sync', status: 'Completed', detail: `${result.copied} copied · ${result.unchanged} unchanged` }, ...jobs]; plan = null; } catch (error) { syncNotice = `Nothing was reported as complete: ${String(error)}`; jobs = [{ kind: 'Library sync', status: 'Failed', detail: String(error) }, ...jobs]; } }
  async function compareCores() { comparison = null; compareNotice = ''; try { comparison = await compareMedia(leftCore, rightCore); compareNotice = `${comparison.differences.length} supported files compared. Nothing was changed.`; } catch (error) { compareNotice = `Could not compare these media roots: ${String(error)}`; } }
  async function reviewCoreCopy() { coreCopyPlan = null; moveSource = false; approveDeletion = false; try { coreCopyPlan = await planCoreCopy(leftCore, rightCore); compareNotice = `Copy plan ${coreCopyPlan.id} is ready for review. The first core remains untouched.`; } catch (error) { compareNotice = `Could not make a copy plan: ${String(error)}`; } }
  async function runCoreCopy() { if (!coreCopyPlan) return; if (moveSource && !approveDeletion) { compareNotice = 'Confirm that the source will be backed up and deleted before moving.'; return; } if (moveSource && !moveBackupPath.trim()) { compareNotice = 'Choose a visible external backup folder before moving.'; return; } try { const result = moveSource ? await executeCoreMove(leftCore, rightCore, coreCopyPlan.id, moveBackupPath, copyReportPath) : await executeCoreCopy(leftCore, rightCore, coreCopyPlan.id, copyReportPath); compareNotice = `Verified ${moveSource ? 'move' : 'copy'}: ${result.copied} copied, ${result.unchanged} unchanged. Index: ${result.index_path}`; coreCopyPlan = null; } catch (error) { compareNotice = `Nothing was reported as complete: ${String(error)}`; } }
